...
* On a platform with no wheel, pip falls back to the source distribution. It needs a Rust toolchain to build
  and does not contain the eSpeak-ng data.
...
[NOTE]
====
`pydengjen` 2.0.3 and earlier differ in four ways, all fixed in later wheels. The wheels contain no
...
silently ignored. xref:index.adoc#what-each-model-supports[What each model supports] lists the keys:
