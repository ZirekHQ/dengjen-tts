...
* The executable needs the Microsoft Visual C{plus}{plus} runtime (it imports `VCRUNTIME140.dll`,
  `VCRUNTIME140_1.dll` and `MSVCP140.dll`). Install the
  https://learn.microsoft.com/cpp/windows/latest-supported-vc-redist[Visual C{plus}{plus} Redistributable]
...
`.proto` file. Install https://github.com/fullstorydev/grpcurl[grpcurl], then, from a shell:
...
`examples/grpc/call_sequence.sh` in the https://github.com/ZirekHQ/dengjen-tts[repository] makes the same
...
Install https://jqlang.org/[jq] as well, then run the script
