...
* `--rate`, `--pitch` and `--volume` take 0-100 and leave the audio unprocessed when omitted. The value maps linearly onto a speed of 0.5x-5.5x (so 50 is 3x), a pitch of 0.5-1.5 and a volume gain of 0-1 (a volume of 0 is ignored). `--silence` appends that many milliseconds of silence after each sentence.
