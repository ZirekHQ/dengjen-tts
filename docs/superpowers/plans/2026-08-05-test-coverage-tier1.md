...
- **`cli`'s silently-swallowed stdin JSON-parse error** — on closer reading, logging and continuing isn't a bug for a single malformed request: `get_synthesis_request_from_stdin` errors are caught and `log::error!`-reported before the read loop continues, which is reasonable behavior for a long-running stdin request loop (one malformed request shouldn't kill the session). The design doc's "fix" framing for that case was wrong; retracted, no task added. However, stdin *EOF* specifically is a real, separate, pre-existing defect this plan doesn't fix: once stdin closes, `read_line` returns `Ok(0)` forever, `serde_json::from_str("")` errors every iteration, and `main.rs`'s `for i in 0.. { ... }` loop never terminates — a hot infinite busy-loop flooding logs. Out of scope for Tier 1; would need its own task/plan. (Also not unit-testable here without refactoring the function to accept an injectable `Read` instead of live stdin.)
